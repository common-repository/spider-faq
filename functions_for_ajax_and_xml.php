...
<body id="link"  style="" dir="ltr" class="forceColors">
  <?php
  if(isset($_GET['edit']) && $_GET['edit'] != 'undefined') {
    $edit_id = $_GET['edit'];
  } else {
    $edit_id = 0;
  }
          ...
          <tr>
            ...
            <td style="vertical-align:top">
              <select name="Spider_Faqname" id="Spider_Faqname" style="width:200px;" >
                ...
                {
                ?>
                <option value="<?php echo $arr_Spider_Faq->id; ?>" <?php echo $edit_id == $arr_Spider_Faq->id ? 'selected' : '' ?>><?php echo $arr_Spider_Faq->title; ?></option>
                <?php }?>
              </select>
            </td>
          </tr>
...
<script type="text/javascript">
  function insert_Spider_FAQ() {
    var shortcode='[Spider_FAQ id="'+document.getElementById('Spider_Faqname').value+'" ]';
    if (window.parent.window['wdg_cb_tw/faq']) {
      window.parent['wdg_cb_tw/faq'](shortcode, 0);
      return;
    }
    else {
